@extends('welcome')
@section('keywords', $carousel->title )
@section('description', Illuminate\Support\Str::markdown($carousel->description) )
...
@section('content')
    ...
    <section class="mt-4 px-2 bg-white dark:bg-gray-900 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 rounded-lg">
        <div class="container mx-auto">
            <picture>
                <source srcset="{{ asset('storage/'. $carousel->image_url_mobile) }}" media="(max-width: 640px)">
                <source srcset="{{ asset('storage/'. $carousel->image_url_desktop) }}" media="(min-width: 641px)">
                <img src="{{ asset('storage/'. $carousel->image_url_desktop) }}" class="block w-full rounded-lg" alt="Carousel {{ $carousel->title }}">
            </picture>
            <div class="my-5">
                <h2 class="text-3xl font-bold text-foreground">{{ $carousel->title }}</h2>
                <div class="my-4">
                    <span class="markdown-content">{!! Illuminate\Support\Str::markdown($carousel->description) !!}</span>
                </div>
            </div>
        </div>
    </section>
